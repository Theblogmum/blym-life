...
create policy "Users can view own lifetime" on public.lifetime_purchases for select using (auth.uid() = user_id);
create policy "Service role can manage lifetime" on public.lifetime_purchases for all using (auth.role() = 'service_role');
